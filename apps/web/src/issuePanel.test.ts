...
const TIMEOUT_FETCH_LIFECYCLE = {
  kind: "timeout",
  detail: "Epic validation and coordination status request timed out: Beads command timed out.",
} as const;

describe("deriveEpicWorkflowFetchLifecycle", () => {
  ...
  it("preserves distinct validation and status backend errors", () => {
    expect(
      ...
      kind: "error",
      detail:
        "Epic validation request failed: Issue 'EPIC-404' was not found. Coordination status request failed: Swarm 'swarm-404' was not found.",
    });
  });
...
describe("deriveEpicWorkflowState", () => {
...
describe("collectEpicWorkflowEntries", () => {
...
describe("partitionEpicWorkflows", () => {
...
describe("partitionEpicWorkflows", () => {
  it("partitions and sorts running and ready swarms", () => {
    ...
    expect(partitionEpicWorkflowTrackerSummaries(swarms)).toEqual({
