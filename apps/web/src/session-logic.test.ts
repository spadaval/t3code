...
describe("resolveRunningSessionStopCommand", () => {
  it("uses a hard session stop for stalled sessions", () => {
    expect(
      resolveRunningSessionStopCommand({
        stalledSession: {
          lastActivityAt: "2026-02-23T00:01:00.000Z",
    ...
    ).toBe("thread.session.stop");
  });

  it("uses turn interrupt while the session is still actively running", () => {
    expect(resolveRunningSessionStopCommand({ stalledSession: null })).toBe(
