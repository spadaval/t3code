...
function findDraftThreadById(
  draftThreadsByThreadKey: ReturnType<
    typeof useComposerDraftStore.getState
  >["draftThreadsByThreadKey"],
  threadId: ThreadId,
): DraftThreadState | null {
  for (const draftThread of Object.values(draftThreadsByThreadKey)) {
    if (draftThread.threadId === threadId || draftThread.promotedTo?.threadId === threadId) {
      return draftThread;
    }
  }
  return null;
...
export function useThreadProjectContext(threadId: ThreadId): ThreadProjectContext {
  ...
  const draftThread = useComposerDraftStore((store) =>
    findDraftThreadById(store.draftThreadsByThreadKey, threadId),
  );
