import type { BeadsEpicWorkflowSnapshot, OrchestrationEpicRun } from "@t3tools/contracts";
import { describe, expect, it } from "vitest";

import {
  buildEpicWorkflowRunEntries,
  describeEpicWorkflowActionCopy,
  resolveEpicOutputTarget,
  resolvePrimaryActionOutputTarget,
  selectEpicWorkflowRunEntry,
...
function makeEpic(
  ...
  overrides: Partial<BeadsEpicWorkflowSnapshot> = {},
...
describe("epicCoordinatorUi", () => {
  it("groups active, failed latest, and history runs into coordinator sections", () => {
    ...
    const entries = buildEpicWorkflowRunEntries([activeEpic, failedEpic, historyEpic]);
  ...
  it("resolves coordinator selection by run id, then epic id, then active/failure/history precedence", () => {
    ...
    expect(selectEpicWorkflowRunEntry({ entries, runId: "run-done" })?.run.runId).toBe("run-done");
    expect(selectEpicWorkflowRunEntry({ entries, epicId: "EPIC-B" })?.run.runId).toBe("run-failed");
    expect(selectEpicWorkflowRunEntry({ entries })?.run.runId).toBe("run-active");
  });
