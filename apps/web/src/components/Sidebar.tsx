...
import { useEpicWorkflowActionRunner } from "../hooks/useEpicCoordinatorActionRunner";
...
import { useSidebarEpicData } from "./sidebar/useSidebarEpicData";
...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  const allSidebarThreads = sidebarThreads;
  ...
  const { epicIssueIds, epicTitleByIssueId, issueTitleByIssueId, epicIssueStatusById } =
    useSidebarEpicData({
      cwd: project.cwd,
      projectExpanded,
      projectEpicRuns,
  ...
  const runsByEpicIssueId = useMemo(() => {
    const next = new Map<string, OrchestrationEpicRun[]>();
    for (const run of projectEpicRuns) {
      const existing = next.get(run.epicIssueId);
      if (existing) {
        existing.push(run);
      } else {
        next.set(run.epicIssueId, [run]);
      }
    }
    return next as ReadonlyMap<string, readonly OrchestrationEpicRun[]>;
  }, [projectEpicRuns]);
  const canOpenIssueInSidebar = activeRouteThread !== null;
  const resolvedEpicWorkflowModelSelection = useMemo(
    () =>
  ...
  const resolvedEpicWorkflowRuntimeMode =
    activeRouteFullThread?.runtimeMode ?? DEFAULT_RUNTIME_MODE;
  const epicActionRunner = useEpicWorkflowActionRunner({
    cwd: project.cwd,
    projectId: project.id,
    modelSelection: resolvedEpicWorkflowModelSelection,
    runtimeMode: resolvedEpicWorkflowRuntimeMode,
  ...
  const canStartNewRunByIssueId = useMemo(() => {
    ...
    for (const epicIssueId of epicIssueIds) {
      const hasActiveRun = (runsByEpicIssueId.get(epicIssueId) ?? []).some((run) =>
        isActiveRunStatus(run.status),
      );
      next.set(epicIssueId, epicIssueStatusById.get(epicIssueId) !== "closed" && !hasActiveRun);
    }
    return next as ReadonlyMap<string, boolean>;
  }, [epicIssueIds, epicIssueStatusById, runsByEpicIssueId]);
  ...
  const openEpicRunFromContextMenu = useCallback(
    async (epicIssueId: string) => {
      if (!canStartNewRunByIssueId.get(epicIssueId)) {
        return;
      }

      await epicActionRunner.runAction({
        kind: "start_epic_run",
        epicIssueId,
      });
    },
    [canStartNewRunByIssueId, epicActionRunner],
  );
