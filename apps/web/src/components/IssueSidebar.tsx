...
import { buildCanonicalIssuesRouteSearch } from "~/issuesRouteSearch";
...
export function IssueSidebar(props: {
  ...
  useEffect(() => {
    ...
    void navigate({
      ...
      replace: true,
      search: (previous) => buildIssueRightPaneSearch(previous, initialLinkedIssueId),
    });
  }, [
    environmentId,
  ...
  const openLinkedThread = useCallback(() => {
    ...
    void navigate({
      ...
      search: (previous) => buildIssueRightPaneSearch(previous, selectedIssueId),
    });
  }, [linkedThreads, navigate, selectedIssueId]);
  ...
  const onSelectIssue = useCallback(
    (issueId: string) => {
      ...
      void navigate({
        ...
        params: { environmentId, threadId },
        replace: true,
        search: (previous) => buildIssueRightPaneSearch(previous, issueId),
      });
    },
  ...
  const openIssueInTracker = useCallback(() => {
    ...
    const epicId = selectedIssueIsEpic
      ? selectedIssueId
      : selectedIssueParent && isEpicIssueType(selectedIssueParent.issueType)
        ? selectedIssueParent.id
        : null;
    void navigate({
      to: "/projects/$projectId/issues" as never,
      params: { projectId: project.id } as never,
      search: buildCanonicalIssuesRouteSearch({
        epicId,
        issueId: selectedIssueId,
      }) as never,
    });
  }, [navigate, project, selectedIssueId, selectedIssueIsEpic, selectedIssueParent]);
