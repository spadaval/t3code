  ...
  return {
    ...
    orchestration: {
      ...
      getEpicWorkflowDetail: (async () => {
        throw new Error("Not implemented in browser test.");
      }) as EnvironmentApi["orchestration"]["getEpicWorkflowDetail"],
      launchPlanImplementation: (async () => {
        throw new Error("Not implemented in browser test.");
      }) as EnvironmentApi["orchestration"]["launchPlanImplementation"],
      cancelPlanImplementationLaunch: (async () => {
        throw new Error("Not implemented in browser test.");
      }) as EnvironmentApi["orchestration"]["cancelPlanImplementationLaunch"],
      retryPlanImplementationLaunch: (async () => {
        throw new Error("Not implemented in browser test.");
      }) as EnvironmentApi["orchestration"]["retryPlanImplementationLaunch"],
      startEpicRun: (async () => {
        throw new Error("Not implemented in browser test.");
      }) as EnvironmentApi["orchestration"]["startEpicRun"],
      stopEpicRun: (async () => {
        throw new Error("Not implemented in browser test.");
      }) as EnvironmentApi["orchestration"]["stopEpicRun"],
...
function createDesktopBridgeStub(
  ...
  return {
    ...
    getAppBranding: vi.fn().mockResolvedValue({
      appName: "T3 Code",
      displayName: "T3 Code",
      accentColor: null,
    }),
    ...
    ensureSshEnvironment: vi.fn().mockRejectedValue(new Error("Not implemented in browser test.")),
    disconnectSshEnvironment: vi.fn().mockResolvedValue(undefined),
    fetchSshEnvironmentDescriptor: vi
      .fn()
      .mockRejectedValue(new Error("Not implemented in browser test.")),
    bootstrapSshBearerSession: vi
      .fn()
      .mockRejectedValue(new Error("Not implemented in browser test.")),
    fetchSshSessionState: vi.fn().mockRejectedValue(new Error("Not implemented in browser test.")),
    issueSshWebSocketToken: vi
      .fn()
      .mockRejectedValue(new Error("Not implemented in browser test.")),
    onSshPasswordPrompt: vi.fn(() => () => undefined),
    ...
    setTailscaleServeEnabled: vi.fn().mockResolvedValue({
      mode: "local-only",
      endpointUrl: null,
      advertisedHost: null,
      tailscaleServeEnabled: false,
    ...
    getAdvertisedEndpoints: vi.fn().mockResolvedValue([]),
...
function createManagedIssueSidebarRpcResolver(options?: {
  ...
  return (body: NormalizedWsRpcRequestBody): unknown | undefined => {
    switch (body._tag) {
      ...
      case "beads.getProjectRunSummary":
        throw new Error("Managed issue sidebar should not request beads.getProjectRunSummary.");
      case "beads.getEpicCoordinationDetail":
        throw new Error(
          "Managed issue sidebar should not request beads.getEpicCoordinationDetail.",
        );
      case ORCHESTRATION_WS_METHODS.getEpicWorkflowDetail:
        return {
          epicId: EPIC_ID,
          epicTitle: "Epic 1",
          issue: null,
          ...
          activeExecutionId: MANAGED_EXECUTION_ID,
...
function createDraftQuickLaunchRpcResolver() {
  return (body: NormalizedWsRpcRequestBody): unknown | undefined => {
    switch (body._tag) {
      case BEADS_WS_METHODS.queryIssues:
        if (body.mode !== "ready") {
          throw new Error(`Unexpected draft quick launch issue mode: ${String(body.mode)}.`);
        }
        return {
          issues: [
            {
      ...
      case BEADS_WS_METHODS.getIssues:
        return {
          issues: [
            {
      ...
      case BEADS_WS_METHODS.getEpicIssueSummaries:
        if (body.epicIssueId !== QUICK_LAUNCH_EPIC_ID) {
          throw new Error(
            `Unexpected epic issue summaries request for ${String(body.epicIssueId)}.`,
          );
        }
        return {
      ...
      case "beads.getProjectRunSummary":
        throw new Error("Draft quick launch should not request beads.getProjectRunSummary.");
      case "beads.getEpicCoordinationDetail":
        throw new Error("Draft quick launch should not request beads.getEpicCoordinationDetail.");
      default:
        return undefined;
    }
  };
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("does not wait for draft beads refresh work before dispatching a send", async () => {
    const draftId = "draft-send-with-pending-ready-issues";
    setDraftThreadWithoutWorktree(draftId);
    ...
    let resolveReadyIssues!: (value: unknown) => void;
    const readyIssuesPromise = new Promise((resolve) => {
      resolveReadyIssues = resolve;
    });
    const draftQuickLaunchResolver = createDraftQuickLaunchRpcResolver();
    ...
    const mounted = await mountChatView({
      ...
      resolveRpc: (body) => {
        if (body._tag === BEADS_WS_METHODS.queryIssues) {
          return readyIssuesPromise;
        }
        if (body._tag === ORCHESTRATION_WS_METHODS.dispatchCommand) {
          return dispatchPromise;
        }
        return draftQuickLaunchResolver(body);
      },
    });

    try {
      useComposerDraftStore.getState().setPrompt(DraftId.make(draftId), "Ship it");
      await waitForLayout();
