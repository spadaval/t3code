...
const ISSUE_LIST_SKELETON_KEYS = ["a", "b", "c", "d", "e", "f"] as const;
    ...
    return (
      <div className={cn("flex flex-col h-full", className)}>
        ...
        <div className="flex-1 p-4 space-y-1">
          {ISSUE_LIST_SKELETON_KEYS.map((key) => (
            <div
              key={`skeleton-${key}`}
          ...
          <div className="py-1">
            ...
            {canShowMore ? (
              <div className="border-t border-border/40 px-3 py-3">
                <Button variant="outline" size="sm" className="w-full" onClick={onShowMore}>
                  Show more issues
                </Button>
              </div>
            ) : null}
...
function IssueListHeader({
  ...
  const countLabel = loading ? "Loading..." : `${totalCount}`;
