...
export function createWsNativeApi(): NativeApi {
  ...
  const api: NativeApi = {
    ...
    git: {
      ...
      workingTree: rpcClient.git.workingTree,
      currentPullRequest: rpcClient.git.currentPullRequest,
    ...
    beads: {
      ...
      getSwarmSupport: rpcClient.beads.getSwarmSupport,
      getIssueGraph: rpcClient.beads.getIssueGraph,
      getEpicSwarm: rpcClient.beads.getEpicSwarm,
    ...
    orchestration: {
      ...
      launchPlanImplementation: (input) =>
        rpcClient.orchestration.launchPlanImplementation(input as never),
      ...
      resumePausedSwarmRun: (input) => rpcClient.orchestration.resumePausedSwarmRun(input as never),
      runNextSwarmTask: (input) => rpcClient.orchestration.runNextSwarmTask(input as never),
      retrySwarmTaskExecution: (input) =>
        rpcClient.orchestration.retrySwarmTaskExecution(input as never),
