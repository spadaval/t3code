  ...
  (it) => {
    describe("checkCodexProviderStatus", () => {
      ...
      it.effect("returns actionable status details when the app-server probe times out", () =>
        Effect.gen(function* () {
          const fiber = yield* checkCodexProviderStatus(
        ...
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            assert.match(
              ...
              /^Codex CLI \(`t3code_codex_missing_\d+_\d+`\) is not installed or not on PATH:/,
            );
            assert.match(codexPersonal?.message ?? "", /Failed to spawn Codex App Server process/);
          }).pipe(Effect.provide(runtimeServices));
        }),
      );
