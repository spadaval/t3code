...
import {
  ...
  type OrchestrationSubagentEntry,
...
const REQUIRED_SNAPSHOT_PROJECTORS = [
  ...
  ORCHESTRATION_PROJECTOR_NAMES.threadSubagents,
...
function mapSubagentEntryRow(
  row: Schema.Schema.Type<typeof ProjectionThreadSubagentEntryDbRowSchema>,
...
function mapSubagentRunRow(
  row: Schema.Schema.Type<typeof ProjectionThreadSubagentRunDbRowSchema>,
  entries: ReadonlyArray<OrchestrationSubagentEntry>,
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const repositoryIdentityResolver: Option.Option<RepositoryIdentityResolverShape> =
    yield* Effect.serviceOption(RepositoryIdentityResolver);
  const resolveRepositoryIdentity = (
    workspaceRoot: string,
  ): Effect.Effect<RepositoryIdentity | null> => {
    if (Option.isNone(repositoryIdentityResolver)) {
      return Effect.succeed(null);
    }
    return repositoryIdentityResolver.value.resolve(workspaceRoot);
  };
  ...
  const resolveRepositoryIdentitiesForProjects = Effect.fn(
    ...
    const repositoryIdentityByWorkspaceRoot = new Map(
      yield* Effect.forEach(
        ...
        (workspaceRoot) =>
          resolveRepositoryIdentity(workspaceRoot).pipe(
  ...
  const listThreadSubagentRunRows = SqlSchema.findAll({
    ...
    execute: () =>
      sql`
        ...
        WHERE recent_rank <= ${MAX_THREAD_SUBAGENT_RUNS}
        ORDER BY thread_id ASC, started_at ASC, run_id ASC
      `,
  });

  const listThreadSubagentEntryRows = SqlSchema.findAll({
    Request: Schema.Void,
    ...
    execute: () =>
      sql`
        ...
        SELECT
          ...
          entry.title,
          entry.text,
          entry.payload_json AS "payload",
          entry.created_at AS "createdAt"
        FROM (
          SELECT
            entry.*,
            ROW_NUMBER() OVER (
              PARTITION BY entry.run_id
              ORDER BY entry.created_at DESC, entry.entry_id DESC
            ) AS recent_rank
          FROM projection_thread_subagent_entries AS entry
          INNER JOIN capped_runs AS run
            ON run.run_id = entry.run_id
        ) AS entry
        WHERE entry.recent_rank <= ${MAX_SUBAGENT_RUN_ENTRIES}
        ORDER BY entry.run_id ASC, entry.created_at ASC, entry.entry_id ASC
      `,
  });
  ...
  const listEpicRunRowsByProject = SqlSchema.findAll({
  ...
  const listEpicIssueExecutionRowsByProjectEpic = SqlSchema.findAll({
    Request: Schema.Struct({
      projectId: ProjectId,
      epicIssueId: Schema.String,
    }),
    Result: ProjectionEpicIssueExecutionDbRowSchema,
    execute: (request) =>
      sql`
        SELECT
          executions.execution_id AS "executionId",
          executions.run_id AS "runId",
          executions.issue_id AS "issueId",
          executions.worker_thread_id AS "workerThreadId",
          executions.sequence_number AS "sequenceNumber",
          executions.status,
          executions.workspace_key AS "workspaceKey",
          executions.workspace_path AS "workspacePath",
          CASE
            WHEN executions.failure_kind IS NULL OR executions.failure_message IS NULL THEN NULL
            ELSE json_object(
              'kind', executions.failure_kind,
              'message', executions.failure_message,
              'issueId', executions.failure_issue_id,
              'executionId', executions.failure_execution_id,
              'workerThreadId', executions.failure_worker_thread_id
            )
          END AS "failureContext",
          executions.requested_at AS "requestedAt",
          executions.started_at AS "startedAt",
          executions.stop_requested_at AS "stopRequestedAt",
          executions.stopped_at AS "stoppedAt",
          executions.completed_at AS "completedAt",
          executions.failed_at AS "failedAt",
          executions.updated_at AS "updatedAt"
        FROM projection_epic_issue_executions executions
        JOIN projection_epic_runs runs ON runs.run_id = executions.run_id
        WHERE runs.project_id = ${request.projectId}
          AND runs.epic_issue_id = ${request.epicIssueId}
        ORDER BY executions.run_id ASC, executions.sequence_number ASC, executions.execution_id ASC
      `,
  });
    ...
    execute: () =>
      sql`
        SELECT
          ...
          turns.state,
          turns.terminal_source AS "terminalSource",
  ...
  const listThreadSubagentRunRowsByThread = SqlSchema.findAll({
    Request: ThreadIdLookupInput,
    Result: ProjectionThreadSubagentRunDbRowSchema,
    execute: ({ threadId }) =>
      sql`
  ...
  const listThreadSubagentEntryRowsByThread = SqlSchema.findAll({
    Request: ThreadIdLookupInput,
    Result: ProjectionThreadSubagentEntryDbRowSchema,
    execute: ({ threadId }) =>
      sql`
      ...
      .withTransaction(
        Effect.gen(function* () {
          const [
            ...
            listThreadSubagentEntryRows(undefined).pipe(
              Effect.mapError(
                toPersistenceSqlOrDecodeError(
                  "ProjectionSnapshotQuery.getSnapshot:listThreadSubagentEntries:query",
                  "ProjectionSnapshotQuery.getSnapshot:listThreadSubagentEntries:decodeRows",
                ),
              ),
            ),
          ...
          const subagentRunsByThread = new Map<string, Array<OrchestrationSubagentRun>>();
          const subagentEntriesByRun = new Map<string, Array<OrchestrationSubagentEntry>>();
          ...
          for (const row of subagentEntryRows) {
            updatedAt = maxIso(updatedAt, row.createdAt);
            const runEntries = subagentEntriesByRun.get(row.runId) ?? [];
            runEntries.push(mapSubagentEntryRow(row));
            subagentEntriesByRun.set(row.runId, runEntries);
          }

          for (const row of subagentRunRows) {
            updatedAt = maxIso(updatedAt, row.updatedAt);
            const threadRuns = subagentRunsByThread.get(row.threadId) ?? [];
            threadRuns.push(mapSubagentRunRow(row, subagentEntriesByRun.get(row.runId) ?? []));
            subagentRunsByThread.set(row.threadId, threadRuns);
          }
          ...
          const threads: ReadonlyArray<OrchestrationThread> = threadRows.map((row) => ({
            ...
            subagentRuns: subagentRunsByThread.get(row.threadId) ?? [],
      ...
      .withTransaction(
        Effect.all([
          ...
          listPlanImplementationLaunchRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getCommandReadModel:listPlanImplementationLaunches:query",
                "ProjectionSnapshotQuery.getCommandReadModel:listPlanImplementationLaunches:decodeRows",
              ),
            ),
          ),
          listEpicRunRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getCommandReadModel:listEpicRuns:query",
                "ProjectionSnapshotQuery.getCommandReadModel:listEpicRuns:decodeRows",
              ),
            ),
          ),
          listEpicIssueExecutionRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getCommandReadModel:listEpicIssueExecutions:query",
                "ProjectionSnapshotQuery.getCommandReadModel:listEpicIssueExecutions:decodeRows",
              ),
            ),
          ),
          listLatestTurnRows(undefined).pipe(
  ...
  const getEpicWorkflowRuntimeState: ProjectionSnapshotQueryShape["getEpicWorkflowRuntimeState"] = (
    ...
    Effect.gen(function* () {
      const [projectEpicRunRows, epicIssueExecutionRows] = yield* Effect.all([
        listEpicRunRowsByProject({ projectId: input.projectId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getEpicWorkflowRuntimeState:listEpicRuns:query",
              "ProjectionSnapshotQuery.getEpicWorkflowRuntimeState:listEpicRuns:decodeRows",
            ),
          ),
        ),
        listEpicIssueExecutionRowsByProjectEpic({
          projectId: input.projectId,
          epicIssueId: input.epicIssueId,
        }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getEpicWorkflowRuntimeState:listEpicIssueExecutions:query",
              "ProjectionSnapshotQuery.getEpicWorkflowRuntimeState:listEpicIssueExecutions:decodeRows",
            ),
          ),
        ),
      ]);
      ...
      return {
        projectEpicRuns: projectEpicRunRows.map(mapEpicRunRow),
        epicIssueExecutions: epicIssueExecutionRows.map(mapEpicIssueExecutionRow),
      };
    });

  const listPendingCheckpointCaptures: ProjectionSnapshotQueryShape["listPendingCheckpointCaptures"] =
    () =>
      listPendingCheckpointCaptureRows(undefined).pipe(
        Effect.mapError(
          toPersistenceSqlOrDecodeError(
            "ProjectionSnapshotQuery.listPendingCheckpointCaptures:query",
            "ProjectionSnapshotQuery.listPendingCheckpointCaptures:decodeRows",
          ),
        ),
      );

  const listProjectLinkedIssueThreads: ProjectionSnapshotQueryShape["listProjectLinkedIssueThreads"] =
    (projectId) =>
      listThreadRowsByProject({ projectId }).pipe(
        Effect.mapError(
          toPersistenceSqlOrDecodeError(
            "ProjectionSnapshotQuery.listProjectLinkedIssueThreads:query",
            "ProjectionSnapshotQuery.listProjectLinkedIssueThreads:decodeRows",
          ),
        ),
        Effect.map((rows) =>
          rows.map((row) => ({
  ...
  const getThreadDetailById: ProjectionSnapshotQueryShape["getThreadDetailById"] = (threadId) =>
    Effect.gen(function* () {
      const [
        ...
        listThreadSubagentRunRowsByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadDetailById:listSubagentRuns:query",
              "ProjectionSnapshotQuery.getThreadDetailById:listSubagentRuns:decodeRows",
            ),
          ),
        ),
        listThreadSubagentEntryRowsByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadDetailById:listSubagentEntries:query",
              "ProjectionSnapshotQuery.getThreadDetailById:listSubagentEntries:decodeRows",
            ),
          ),
        ),
      ...
      const thread = {
        ...
        subagentRuns: subagentRunRows.map((row) =>
          mapSubagentRunRow(row, subagentEntriesByRun.get(row.runId) ?? []),
        ),
