...
const makePlanImplementationWorkflow = Effect.gen(function* () {
  ...
  const getSourceContext = (launch: ProjectionPlanImplementationLaunch) =>
    Effect.all({
      sourceThread: projectionSnapshotQuery
        .getThreadDetailById(launch.sourceThreadId)
        .pipe(
          Effect.mapError((error) =>
            workflowError("getSourceContext", truncateDetail(toErrorMessage(error)), error),
          ),
        ),
      project: projectionSnapshotQuery
        .getProjectShellById(launch.projectId)
        .pipe(
          Effect.mapError((error) =>
            workflowError("getSourceContext", truncateDetail(toErrorMessage(error)), error),
          ),
        ),
      targetThread: projectionSnapshotQuery
        .getThreadDetailById(launch.targetThreadId)
        .pipe(
          Effect.mapError((error) =>
            workflowError("getSourceContext", truncateDetail(toErrorMessage(error)), error),
          ),
      ...
      Effect.flatMap(({ sourceThread, project, targetThread }) => {
        ...
        const sourcePlan =
          sourceThread.value.proposedPlans.find((plan) => plan.id === launch.sourcePlanId) ?? null;
        if (!sourcePlan) {
          return Effect.fail(
            workflowError(
              "getSourceContext",
        ...
        return Effect.succeed({
          sourceThread: sourceThread.value,
          sourcePlan,
          project: project.value,
          targetThread: Option.getOrNull(targetThread),
        } as const);
      }),
    );
